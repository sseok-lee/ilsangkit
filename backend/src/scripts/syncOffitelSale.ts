...
export async function syncOffitelSaleByLawd(lawdCd: string, dealYmd: string, serviceKey: string, regionMap: Map<string, { city: string; district: string }>,
  ...
  await runSync(CATEGORY, async (stats) => {
    ...
    for (const lawdCd of lawdCodes) {
      for (const ym of ymList) {
        try {
          await syncOffitelSaleByLawd(lawdCd, ym, serviceKey, regionMap, stats);
        } catch (error) {
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(__filename)) {
  installRuntimeGuard({ maxMinutes: 20, name: 'syncOffitelSale', prisma });
  main().catch((error) => {
