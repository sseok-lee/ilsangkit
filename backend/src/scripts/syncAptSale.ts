...
export async function syncAptSaleByLawd(
  lawdCd: string,
  dealYmd: string,
  serviceKey: string,
...
async function main(): Promise<void> {
  ...
  await runSync(CATEGORY, async (stats) => {
    ...
    for (const lawdCd of lawdCodes) {
      for (const ym of ymList) {
        try {
          await syncAptSaleByLawd(lawdCd, ym, serviceKey, regionMap, stats);
        } catch (error) {
  ...
  // 지번/thin buildingName 제외 — SEO 저품질 URL 제출 방지
  ...
  if (validBuildings.length > 0) {
    const urls = buildRealEstateUrlsV2(validBuildings.map(b => ({
      realEstateType: 'apt-sale' as const,
      city: b.city,
      district: b.district,
      buildingName: b.buildingName,
    })));
    console.info(`[aptSale] IndexNow: ${buildings.length} candidates → ${validBuildings.length} valid (filtered ${buildings.length - validBuildings.length})`);
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(__filename)) {
  installRuntimeGuard({ maxMinutes: 20, name: 'syncAptSale', prisma });
  main().catch((error) => {
