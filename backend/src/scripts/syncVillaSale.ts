...
export async function syncVillaSaleByLawd(lawdCd: string, dealYmd: string, serviceKey: string, regionMap: Map<string, { city: string; district: string }>,
  ...
  await runSync(CATEGORY, async (stats) => {
    ...
    for (const lawdCd of lawdCodes) {
      for (const ym of ymList) {
        try {
          await syncVillaSaleByLawd(lawdCd, ym, serviceKey, regionMap, stats);
        } catch (error) {
