...
export function transformWifiData(row: WifiCSVRow): TransformedWifi | null {
  ...
  const shortCity = CITY_NORMALIZATION_MAP[rawCity] || rawCity || 'unknown';
  // normalizeRegionName은 광주/전남 변종만 통합하고 그 외(서울 등)는 passthrough.
  const city = normalizeRegionName(shortCity, district).city;
