...
export async function syncOffitelRentByLawd(lawdCd: string, dealYmd: string, serviceKey: string, regionMap: Map<string, { city: string; district: string }>,
  ...
  await runSync(CATEGORY, async (stats) => {
    ...
    for (const lawdCd of lawdCodes) {
      for (const ym of ymList) {
        try {
          await syncOffitelRentByLawd(lawdCd, ym, serviceKey, regionMap, stats);
        } catch (error) {
