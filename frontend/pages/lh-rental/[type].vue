...
if (!typeMeta) {
  throw createError({ statusCode: 404, statusMessage: '존재하지 않는 LH 임대 카테고리입니다' })
}

const title = `${typeMeta.label} | LH 임대 | 일상킷`
const description = `${typeMeta.label} - ${typeMeta.description}`
const canonicalUrl = `${SITE_URL}/lh-rental/${type}`
...
setBreadcrumbSchema([
  { name: '홈', url: SITE_URL },
  { name: 'LH 임대', url: `${SITE_URL}/lh-rental` },
