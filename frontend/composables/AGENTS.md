...
## Purpose
Nuxt auto-import composable 함수. 데이터 fetch, 상태 관리, 외부 SDK(Kakao Map, Analytics, MSW) 통합을 담당한다. 모든 composable은 `useXxx` 네이밍. 시설, 부동산, 청약, 가이드, 쓰레기 등 각 도메인별 데이터 로드 함수 제공.
