...
<template>
  <HardLink :to="to" class="block bg-white rounded-xl border border-line p-4 shadow-card hover:border-primary/30 transition-[box-shadow,border-color]">
    <div class="flex items-center gap-2 mb-2">
      ...
      <span v-if="item.bidRound" class="text-caption text-faint font-display tabular-nums">{{ item.bidRound }}차</span>
    </div>
    <p class="text-sm font-semibold text-strong truncate">{{ item.address }}</p>
    <p class="text-caption text-muted mt-0.5">{{ item.usage ?? USAGE_GROUP_LABEL[item.usageGroup] }} · 📍{{ item.district }}</p>
    <div class="mt-3 flex items-end justify-between gap-2">
      <div class="min-w-0">
        <p class="text-caption text-faint">감정가</p>
        <p class="text-sm font-bold text-strong font-display tabular-nums">{{ apslDisplay }}</p>
        <p v-if="showMinBid" class="text-caption text-muted mt-0.5">
          최저가 <span class="font-display tabular-nums">{{ minDisplay }}</span><span v-if="hasDiscount" class="ml-1 font-medium text-emerald-600">{{ discount }}</span>
        </p>
      </div>
      <p class="text-caption text-muted whitespace-nowrap font-display tabular-nums">{{ formatAuctionDate(item.bidCloseDtm) }} 마감</p>
    </div>
  </HardLink>
</template>
