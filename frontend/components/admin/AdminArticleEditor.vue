<template>
  <div class="flex flex-col gap-4">
    ...
    <div>
      <label for="admin-editor-title" class="block text-xs font-medium text-muted mb-1">제목</label>
      <input
    ...
    <div>
      <label for="admin-editor-summary" class="block text-xs font-medium text-muted mb-1">요약</label>
      <textarea
        id="admin-editor-summary"
    ...
    <div>
      <label for="admin-editor-keywords" class="block text-xs font-medium text-muted mb-1">키워드 (쉼표 구분)</label>
      <input
        id="admin-editor-keywords"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
      <div>
        <label for="admin-editor-content" class="block text-xs font-medium text-muted mb-1">본문 (마크다운)</label>
        <textarea
      ...
      <div>
        ...
        <div
          ...
          class="
            border border-line rounded-md p-3 min-h-[20rem] max-h-[32rem] overflow-y-auto
    ...
    <div class="flex flex-wrap gap-2 pt-2 border-t border-line">
      <button
        ...
        class="px-3 py-2 rounded-md text-sm font-medium bg-primary text-white"
        @click="$emit('save', patch)"
      >
        저장
      </button>
      <button
        ...
        class="px-3 py-2 rounded-md text-sm font-medium bg-emerald-600 text-white"
        @click="$emit('publish')"
      >
        발행
      </button>
      ...
      <button
        ...
        class="px-3 py-2 rounded-md text-sm font-medium bg-amber-100 text-amber-700"
        @click="$emit('reject')"
      >
        반려
      </button>
      ...
      <button
        ...
        class="px-3 py-2 rounded-md text-sm font-medium bg-red-50 text-red-700 ml-auto"
        @click="$emit('delete')"
      >
        삭제
      </button>
...
import type { AdminArticleDetail, AdminArticlePatch } from '~/composables/useAdminArticles'

const props = defineProps<{ article: AdminArticleDetail }>()

defineEmits<{
  save: [patch: AdminArticlePatch]
  publish: []
  unpublish: []
  reject: []
  delete: []
...
watch(
  ...
  (article) => {
    ...
    draftSummary.value = article.summary
    draftKeywords.value = article.keywords ?? ''
...
const patch = computed<AdminArticlePatch>(() => ({
