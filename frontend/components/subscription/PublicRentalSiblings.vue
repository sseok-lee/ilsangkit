<template>
  <SectionBlock heading="같은 단지 다른 평형/유형" subtext="동일 단지의 다른 면적 또는 공급유형 매물입니다.">
    ...
    <div v-if="siblings.length === 0" class="rounded-xl bg-background-light p-6 text-center text-muted text-sm">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      <NuxtLink
        ...
        :key="item.id"
        :to="linkFor(item)"
        class="block rounded-xl border border-line-2 bg-white p-4 hover:border-primary hover:shadow-md transition-all"
      >
        <div class="flex items-center gap-2 mb-2">
          <span :class="badgeClass(item.rentalType)">{{ item.rentalType }}</span>
          <span class="text-xs text-muted">{{ fmtArea(item.exclusiveArea) }}</span>
        </div>
        <p class="text-sm font-bold text-strong font-display tabular-nums">
          {{ fmtDeposit(item.depositAmount) }}
        </p>
        <p v-if="!isJeonseRental(item.monthlyRent)" class="text-xs text-ink mt-0.5">
