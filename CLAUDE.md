...
### Users
- **생활정보 검색자**: 근처 시설(화장실, 주차장, AED, 약국 등)을 빠르게 찾으려는 일반 시민. 이동 중 모바일에서 주로 사용.
- **부동산 정보 탐색자**: 실거래가, 청약 정보를 비교하려는 예비 매수자/임차인. 데스크톱+모바일 혼용.

### Brand Personality
- **3단어**: 실용적, 신뢰감, 깔끔함
- **감정 목표**: 안심감 + 편리함 — "필요한 정보를 바로 찾을 수 있다"는 신뢰감

### Aesthetic Direction
- 실용적 미니멀리즘. 정보 밀도를 적절히 유지하면서 시각적 노이즈를 최소화.
- 라이트 전용. 독자적 방향.
- **안티**: 관공서 포털(딱딱/구식), AI 생성 디자인(그라데이션 과다, 네온, 글래스모피즘)

### Design Principles
1. **정보 우선**: 장식보다 정보. 모든 시각 요소는 정보 전달을 돕거나 비켜야 한다.
2. **즉시 이해**: 페이지 도착 0.5초 안에 원하는 정보 위치를 알 수 있어야 한다.
3. **신뢰의 디자인**: 데이터 출처가 명확하고 UI가 안정적으로 느껴져야 한다.
4. **효율적 밀도**: 스캔 가능한 레이아웃, 적정 정보 밀도.
5. **모바일 네이티브**: 터치 친화적, 빠른 로딩 우선.

## 하네스: 정보성 사이트 운영

**목표:** 데이터 소스 탐색부터 SEO/수익화 최적화까지 콘텐츠 생산 풀사이클 자동화

**트리거:** 새 카테고리 기획, 데이터 소스 탐색, 페이지 설계, SEO/수익화 전략 수립 등 콘텐츠 생산 파이프라인 요청 시 `info-site-orchestrator` 스킬을 사용하라. 단순 질문은 직접 응답 가능.

**변경 이력:**
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-04-13 | 초기 구성 | 전체 | 정보성 사이트 콘텐츠 생산 자동화 하네스 구축 |
| 2026-04-13 | ui-ux-layout-designer 에이전트 추가 | agents/, skills/, orchestrator | 메인/상세/허브/메뉴 UI/UX 구조 설계 담당 추가 |
