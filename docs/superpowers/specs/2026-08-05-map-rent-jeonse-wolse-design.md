...
**한 줄 유지 — 전세만.** 목록처럼 두 줄을 넣으면 라벨 폭이 두 배 가까이 되고, 충돌 판정이 `text.length` 로 상자를 잡으므로(`useMapOverlays.ts:134`) 점으로 접히는 마커가 늘어난다. 지도의 마커는 훑어보는 용도이고, 전세가는 건물 간 비교가 되는 단일 숫자라 이 용도에 맞는다. 월세는 보증금과 월세액 두 숫자라 애초에 마커에 얹기 부적합하다.
