...
#### 카테고리 탭 페이지 (/toilet, /wifi, /clothes, /kiosk, /trash)

```
┌─ Mobile (< 640px) ──────┐    ┌─ Desktop (> 1024px) ─────────────────────────────┐
│ 일상킷 (로고)      ≡    │    │ 일상킷   화장실 쓰레기 와이파이 의류 발급기      │
├─────────────────────────┤    ├─────────────────────────────────────────────────┤
│ 🔍 검색...   [📍 현재위치]│    │ 🔍 검색어 입력...   [📍 현재위치로 검색]         │
├─────────────────────────┤    ├─────────────────────────────────────────────────┤
│ [목록 🔘] [지도 ○]      │    │                                                 │
│                         │    │  ┌─────────────────┬───────────────────────┐   │
│ ┌───────────────────┐   │    │  │                 │                       │   │
...
│  ─────────────────────────────────────    │
│  인기 지역                                 │
│  서울 강남구 | 서울 송파구 | 부산 해운대 ... │
└────────────────────────────────────────────┘
```

**사용자 액션:**
- 검색어 입력 → 검색 결과 페이지 (/search)
- 카테고리 클릭 → 해당 카테고리 탭 페이지 (/toilet, /wifi 등)
- 현재 위치 버튼 → 위치 권한 요청 → 주변 검색
- 지역 클릭 → 검색 결과 페이지 (/search?keyword=지역명)
